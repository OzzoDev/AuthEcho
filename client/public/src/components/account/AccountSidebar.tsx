...
interface Props {
  ...
  setCurrentTab: (currentTab: AccountDashboardTabName) => void;
}

export default function AccountSidebar({ tabs, currentTab, setCurrentTab }: Props) {
  const [dashboardTabs, setDashBoardTabs] = useState<AccountDashboardTab[]>(tabs);
  ...
  const handleTabClick = (tabName: AccountDashboardTabName) => {
